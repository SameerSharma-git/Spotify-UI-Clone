...
async function updateAlbums() {
    ...
    foldersDiv.innerHTML = await (await fetch(`${url}/Songs/`)).text()
    ...
    for (let i = 0; i < anchors.length; i++) {
        ...
        if (anchor.href.includes('/songs/') && !anchor.href.endsWith('/songs/') && !anchor.href.endsWith('.mp3')) {
            let foundSubFolder = anchor.href.split('/')[anchor.href.split('/').length - 2]
            Folders.push(foundSubFolder)
        }
    }
    console.log(Folders,`${url}/Songs/`);
    ...
    for (let i = 0; i < Folders.length; i++) {
        ...
        console.log(`${url}/Songs/${Folder}`);
        div.innerHTML = await (await fetch(`${url}/Songs/${Folder}`)).text()
    ...
    for (let i = 0; i < Folders.length; i++) {
        ...
        for (let z = 0; z < subFolders[i].length; z++) {
            ...
            let info = await ((await fetch(`${url}/Songs/${Folder}/${subFolder}/info.json`)).json())
...
async function getSongs(Folder = '', SubFolder) {
    songs = []
    let songData = await fetch(`${url}/Songs/${Folder}/${SubFolder}`)
...
// left Section slide functionality
